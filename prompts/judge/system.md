...
Поля `contrast_rationale`, `controlled_change` и `evaluation_context` являются заявлениями генератора, а не ground truth. Если они противоречат chunks, доверяй chunks.
...
5. `controlled_change` и `contrast_rationale` должны соответствовать фактическому изменению.
6. Если специализированному сценарию нужен `evaluation_context`, его значение должно соответствовать source_document и chunks.
7. Если специализированный prompt требует сохранения текста, запрещены добавление, удаление, перестановка и перефразирование; разрешено менять только границы/группировку.
8. Специализированные правила конкретной метрики имеют приоритет над общими правилами.
...
* обязательный для сценария `evaluation_context` отсутствует или существенно неверен.
...
Верни только JSON строго по schema в конце system message. Не добавляй Markdown, комментарии или дополнительные поля.
