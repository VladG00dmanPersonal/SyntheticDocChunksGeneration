...
| `USE_JUDGE_FEEDBACK_ON_EVEN_ATTEMPTS` | Исправлять отклонённый пример на чётных попытках |
...
По умолчанию `USE_JUDGE_FEEDBACK_ON_EVEN_ATTEMPTS = False`, поэтому каждая
попытка генерирует пример независимо из исходных `system`- и `user`-сообщений.
Если включить флаг, нечётные попытки по-прежнему начинают генерацию с нуля, а
чётные исправляют непосредственно предыдущий пример, отклонённый judge, с
учётом полного verdict. Ошибка JSON-схемы не создаёт feedback-контекст для
следующей попытки; история нескольких отказов не накапливается.
