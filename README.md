...
```json
{
  ...
  "positive": {
    "chunks": ["..."],
    "rationale": "Почему это разбиение ожидаемо лучше",
    "focus": {}
  },
  "negative": {
    "chunks": ["..."],
    "rationale": "Как контролируемое изменение ухудшает разбиение",
    "focus": {}
  },
...
- `rationale` — качественное объяснение ожидаемого направления, а не результат
  измерения.
- `focus` — индексы целевых чанков/границ или проверяемый факт; точный контракт
  зависит от сценария.
- `controlled_change` — минимальное вмешательство, изолирующее проверяемое
  свойство.
- `expected_relation` — гипотеза, что релевантный показатель для `positive`
  окажется выше, чем для `negative`.

Индексы в `focus` начинаются с нуля. Индекс границы `i` обозначает переход между
чанками `i` и `i + 1`. Маркер `||` внутри текста соединяет логически связанные
части одного чанка, например вводную фразу списка и отдельный пункт.
...
- все индексы в `focus` допустимы;
- при требовании одинакового исходного текста не потеряны и не добавлены фрагменты;
- `rationale` не выдаёт ожидаемое направление за уже измеренный score;
